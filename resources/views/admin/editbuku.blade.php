      ...
      <section class="content">
        <div class="container-fluid">
          <div class="col-md-8 col-sm-12 bg-white p-4">
            <form method="post" action="/dashboard/buku/editbuku/edit_process/{{ $Buku->id }}"
              enctype="multipart/form-data">
              @csrf
              <input type="hidden" value="{{ $Buku->id }}" name="id">
              <div class="form-group">
                <label>Judul Artikel</label>
                <input type="string" class="form-control" value="{{ $Buku->judul }}" name="judul"
                  placeholder="Judul artikel">
              </div>
              <div class="form-group">
                <label>Jumlah Buku</label>
                <input type="integer" class="form-control" value="{{ $Buku->jumlah }}" name="jumlah"
                  placeholder="Judul artikel">
              </div>
              <div class="form-group">
                <label>Bahasa Buku</label>
                <input type="string" class="form-control" value="{{ $Buku->bahasa }}" name="bahasa"
                  placeholder="Judul artikel">
              </div>
              <div class="form-group">
                <label>Jenis Buku</label>
                <input type="string" class="form-control" value="{{ $Buku->jenis }}" name="jenis"
                  placeholder="Judul artikel">
              </div>
              <div class="form-group">
                <label>Terbit Buku</label>
                <input type="date" class="form-control" value="{{ $Buku->terbit }}" name="terbit"
                  placeholder="Judul artikel">
              </div>
              <div class="form-group">
                <label>Halaman Buku</label>
                <input type="string" class="form-control" value="{{ $Buku->halaman }}" name="halaman"
                  placeholder="Judul artikel">
              </div>
              ...
              <div class="form-group">
                ...
                @if ($Buku->gambar)
                <img src="{{ asset('storage/'. $Buku->gambar)}}" class="img-preview img-fluid mb-3 col-sm-8 d-block">
                @else
